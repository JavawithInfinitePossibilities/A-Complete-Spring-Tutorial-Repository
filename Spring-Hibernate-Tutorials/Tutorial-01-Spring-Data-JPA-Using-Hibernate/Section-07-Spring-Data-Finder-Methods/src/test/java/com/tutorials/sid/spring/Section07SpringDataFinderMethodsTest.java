...
/**
 * Unit test for simple Section07SpringDataFinderMethods.
 */
@SpringBootTest(classes = Section07SpringDataFinderMethods.class)
public class Section07SpringDataFinderMethodsTest {
    @Autowired
    ...
    void updateCarDetails(){
        CarEntity carEntity=carServices.getCarDetailsById(3010);
        carEntity.setColor("Green");
        carServices.createCarDetails(carEntity);
    }

    @Test
    void getCarDetailsMaker(){
        List<CarEntity> carEntity=carServices.getCarDetailsByMakeName("Toyota");
        carEntity.stream().forEach(car -> {
            System.out.println(car);
        });
    }

    @Test
    void getCarDetailsByColor(){
        List<CarEntity> carEntity=carServices.getCarDetailsByColor("Yellow");
        carEntity.stream().forEach(car -> {
            System.out.println(car);
        });
    }
    ...
    @Test
    void getCarDetailsByYearbetween(){
        List<CarEntity> carEntity=carServices.getCarDetailsByYear(1990,2020);
        carEntity.stream().forEach(car -> {
            System.out.println(car);
        });
    }
