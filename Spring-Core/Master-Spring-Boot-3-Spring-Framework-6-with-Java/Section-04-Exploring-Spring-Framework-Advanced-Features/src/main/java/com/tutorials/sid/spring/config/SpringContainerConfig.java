package com.tutorials.sid.spring.config;

import com.tutorials.sid.spring.game.GameRunner;
import com.tutorials.sid.spring.game.GameingConsole;
import com.tutorials.sid.spring.game.SuperContraGame;
import jakarta.annotation.PostConstruct;
import jakarta.annotation.PreDestroy;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.ComponentScan;
import org.springframework.context.annotation.Configuration;
import org.springframework.context.annotation.Lazy;



/**
 * @author kunmu On 01-07-2026
 */
@Configuration
@ComponentScan("com.tutorials.sid.spring")
@Lazy
public class SpringContainerConfig {

    /*This is called field injection*/
    /*@Autowired
    @Qualifier("marioGame")*/
    private GameingConsole marioGame;

    /*This is called field injection*/
    /*@Autowired*/
    private SuperContraGame superContraGame;

    /*This is called field injection*/
    /*@Autowired*/
    private GameingConsole pacManGame;

    /*This is called setter injection*/
    /*@Autowired
    @Qualifier("marioGame")
    public void setMarioGame(GameingConsole marioGame) {
        System.out.println("MarioGame is injected");
        this.marioGame = marioGame;
    }

    @Autowired
    public void setSuperContraGame(SuperContraGame superContraGame) {
        System.out.println("SuperContraGame is injected");
        this.superContraGame = superContraGame;
    }

    @Autowired
    public void setPacManGame(GameingConsole pacManGame) {
        System.out.println("PacManGame is injected");
        this.pacManGame = pacManGame;
    }*/

    /*This is called constructor injection.
    * Constructor injection is preferred over field injection and setter injection because
    * it makes the dependencies of a class explicit and ensures that the class is always in a valid state after construction.
    * It also allows for easier testing and better encapsulation of the class's dependencies.
    * Constructor injection is also preferred over autowiring because
    * it allows for better control over the initialization process and better separation of concerns.
    * Constructor injection is eager loading, which means that the dependencies are loaded immediately when the bean is created,
    * rather than lazily when they are first accessed. This can improve performance and reduce the risk of null pointer exceptions.
    * To check the lazy loading behavior, you can use the @Lazy annotation on the SpringContainerConfig class.
    * */
    @Autowired
    public SpringContainerConfig(@Qualifier("marioGame")GameingConsole marioGame, SuperContraGame superContraGame, GameingConsole pacManGame) {
        System.out.println("Constructor is called for SpringContainerConfig");
        this.marioGame = marioGame;
        this.superContraGame = superContraGame;
        this.pacManGame = pacManGame;
    }

    /*This is the way to inject the bean by name*/
    @Bean
    public GameRunner marioGameRunner() {
        return new GameRunner(marioGame);
    }

    /*This is the way to inject the bean by type*/
    @Bean
    public GameRunner superContraGameRunner() {
        return new GameRunner(superContraGame);
    }

    /*This is the way to inject the bean by Primary*/
    @Bean
    public GameRunner pacManGameRunner() {
        return new GameRunner(pacManGame);
    }

    @PostConstruct
    public void init() {
        System.out.println("SpringContainerConfig is initialized");
    }

    @PreDestroy
    public void destroy() {
        System.out.println("SpringContainerConfig is destroyed");
    }
}
